...
describe('NodeWorkerPool', () => {
  ...
  describe('构造函数', () => {
    it('应该正确创建指定数量的 worker', () => {
      ...
      expect(nodeWorkerPool.getPoolStatus().runningWorkers).toBe(0)
  ...
  describe('exec', () => {
    it('应该正确执行单个任务', async () => {
      ...
      expect(firstWorker.run).toHaveBeenCalledWith(mockParam, 0, undefined)
    })
    ...
    it('应该正确执行多个任务', async () => {
      ...
      expect(result).toEqual([
        { success: true, data: mockResults[0], index: 0 },
        { success: true, data: mockResults[1], index: 1 },
        { success: true, data: mockResults[2], index: 2 },
      ])
    ...
    it('应该正确处理混合成功和失败的任务', async () => {
      ...
      expect(result).toEqual([
        { success: true, data: mockSuccess, index: 0 },
        { success: false, error: mockError, index: 1 },
      ])
    })
  ...
  describe('terminate', () => {
    it('应该终止所有 worker', () => {
      ...
      // 检查所有 worker 都被终止
      expect(nodeWorkerPool.pool.length).toBe(0)
    })
  ...
  describe('getPoolStatus', () => {
    it('应该正确跟踪正在运行的任务数量', async () => {
      ...
      firstWorker.run = jest.fn().mockImplementation(() => {
        firstWorker.status = WorkerStatusEnum.RUNNING
        return new Promise((resolve) => {
          resolveRun = (value: any) => {
            firstWorker.status = WorkerStatusEnum.WAITING
            resolve(value)
          }
        })
      })
      ...
      // 等待 resolveRun 被赋值
      while (typeof resolveRun !== 'function') {
        await new Promise((r) => setTimeout(r, 0))
      }
      // 检查任务开始执行时的计数
      expect(nodeWorkerPool.getPoolStatus().runningWorkers).toBe(1)
