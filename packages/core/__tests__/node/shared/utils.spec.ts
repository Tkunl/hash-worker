...
describe('utils', () => {
  describe('getArrParts', () => {
    ...
    it('当 size 为 0 或负数时，应该返回空数组', () => {
      const arr = [1, 2, 3]
      expect(getArrParts(arr, 0)).toEqual([])
      expect(getArrParts(arr, -1)).toEqual([])
      expect(getArrParts(arr, -5)).toEqual([])
    })

    it('当 size 不是整数时，应该返回空数组', () => {
      const arr = [1, 2, 3]
      expect(getArrParts(arr, 1.5)).toEqual([])
      expect(getArrParts(arr, 2.7)).toEqual([])
    })
  ...
  describe('generateUUID', () => {
    ...
    it('应该符合模板格式', () => {
      const uuid = generateUUID()
      // 检查第13位是否为4 (根据模板 'xxxxxxxxxxxx4xxxyxxxxxxxxxxxxxxx')
      expect(uuid[12]).toBe('4')
      // 检查第17位是否为8, 9, a, 或 b (根据模板中的 'y')
