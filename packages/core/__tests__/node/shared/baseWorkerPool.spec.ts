...
describe('BaseWorkerPool', () => {
  ...
  describe('exec', () => {
    it('应该能够执行单个任务', async () => {
      ...
      expect(results).toEqual(['result_0'])
    })
    ...
    it('应该能够处理任务执行失败的情况', async () => {
      ...
      let errMsg = ''
      if (typeof results[0] === 'object' && results[0] !== null && 'message' in results[0]) {
        errMsg = (results[0] as Error).message
      }
      expect(errMsg).toBe('Empty chunk')
      expect(results[1]).toBe('result_1')
    })

    it('应该正确管理当前运行的工作线程数量', async () => {
      const params: WorkerReq[] = Array.from({ length: 3 }, () => ({
        chunk: new ArrayBuffer(8),
        strategy: 'md5' as any,
      }))

      const execPromise = workerPool.exec<string>(params)

      // 等待一段时间让任务开始执行
      await new Promise((resolve) => setTimeout(resolve, 5))

      // 检查运行中的工作线程数量
      expect(workerPool.curRunningCount.value).toBe(3)
  ...
  describe('并发控制', () => {
    it('应该限制同时运行的工作线程数量', async () => {
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      const params: WorkerReq[] = Array.from({ length: 5 }, (_, i) => ({
        chunk: new ArrayBuffer(8),
        strategy: 'md5' as any,
      }))
      ...
      // 检查运行中的工作线程数量不超过最大值
      expect(workerPool.curRunningCount.value).toBeLessThanOrEqual(workerPool.maxWorkerCount)
