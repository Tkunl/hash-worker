...
describe('BrowserWorkerWrapper', () => {
  ...
  describe('run 方法', () => {
    ...
    it('应该处理超时配置', async () => {
      ...
      // 模拟超时
      const timeoutPromise = browserWorkerWrapper.run<string>(mockParam, index, { timeout })
      ...
      // 不模拟任何响应，让它超时
      // 模拟超时回调被执行
      const mockSetTimeout = window.setTimeout as jest.MockedFunction<typeof window.setTimeout>
      const timeoutCallback = mockSetTimeout.mock.calls[0][0] as () => void
      timeoutCallback()

      await expect(timeoutPromise).rejects.toThrow('Worker task timeout after 1000ms')
      expect(browserWorkerWrapper.status).toBe(WorkerStatusEnum.WAITING)
    })

    it('应该在成功响应时清除超时', async () => {
      const mockParam = {
        chunk: new ArrayBuffer(8),
        strategy: Strategy.md5,
      }
      ...
      await browserWorkerWrapper.run<string>(mockParam, index, { timeout })
